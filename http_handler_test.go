...
func (suite *HttpHandlerTestSuite) TestPutHandler_Success() {
	...
	httpHandler := httpHandler{suite.annotationsService, suite.producer, suite.originMap, suite.lifecycleMap, suite.messageType, suite.log}
	rec := httptest.NewRecorder()
	router(&httpHandler, &suite.healthCheckHandler, suite.log).ServeHTTP(rec, request)
...
func (suite *HttpHandlerTestSuite) TestGetHandler_Success() {
	...
	rec := httptest.NewRecorder()
	router(&httpHandler{suite.annotationsService, suite.producer, suite.originMap, suite.lifecycleMap, suite.messageType, suite.log}, &suite.healthCheckHandler, suite.log).ServeHTTP(rec, request)
...
func (suite *HttpHandlerTestSuite) TestCount_CountError() {
	...
	request := newRequest("GET", fmt.Sprintf("/content/annotations/%s/__count", annotationLifecycle), "application/json", nil)
	rec := httptest.NewRecorder()
	handler := httpHandler{suite.annotationsService, suite.producer, suite.originMap, suite.lifecycleMap, suite.messageType, suite.log}
